...
namespace GXPEngine
{
	...
	public class MouseHandler
	{
		...
		private Vec2 _last; //where was the mouse last time we checked?
		...
		void HandleOnStep()
		{
			...
			bool isOnTarget = _target.HitTestPoint(Input.mouse);
			...
			if (!_wasMouseDown && Input.GetMouseButton(0))
			{
				...
				_offset.x -= Input.mouse.x;
				_offset.y -= Input.mouse.y;
